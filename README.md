...
An example of this would be in table form like [this](), in Excel format. That is the 
...
to a JSON. My suggestion on how to do this would be to use 
...
This should result in data that looks like [this](). 
...
```
perl -ne 's/"(\d+)"/$1/g; print' input_file.json > output_file.json
```
...
Before you save, remove the extra commas after the text on the second and last lines before the brackets. If you do not, the commas will appear within the visualization. 
```
<script>
...
With a .html file with the code from the beginning and those final lines, you should be able to make the chord diagram visualize whatever you would like.
